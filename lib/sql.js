...
sql.createTable = async function (tickers) {
  ...
  try {
    ...
    // Query Database
    let tablePromises = await Promise.all(
...
sql.postData = async function (tickers) {
  ...
  try {
    ...
    // query Database
    let queryPromises = await Promise.all(
      tickers.map(async ticker => {
        connection = await pool.getConnection(config.connection)
        const query = concatquery(ticker)
        let result = await connection.query(query)
        await connection.release()
        return result
      })
...
sql.getData = async function (tickers, resolution) {
  ...
  try {
    ...
    // query Database
    let querriedCandles = await Promise.all(
      tickers.map(async ticker => {
        connection = await pool.getConnection(config.connection)
