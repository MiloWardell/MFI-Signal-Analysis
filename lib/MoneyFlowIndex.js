...
MFI.getAccuracyMetric = function (querryTickers) {
  ...
  querryTickers.forEach(ticker => {
    ...
    for (let i = 14; i < ticker.candles.length; i++) {
      ...
      if (MFI >= 80) {
        let futureCandles = ticker.candles.slice(i - 5, i)
        let currentPrice = calcTypicalPrice(
          ticker.candles[i].high,
          ticker.candles[i].low,
          ticker.candles[i].close
        )
        let futurePriceAverage = calcAverage(
          futureCandles.map(candle => {
            return calcTypicalPrice(candle.high, candle.low, candle.close)
          })
        )

        if (futurePriceAverage < currentPrice) {
          accOverbought.push(1)
        } else {
          accOverbought.push(0)
        }
      }
    ...
    if (accOverbought.length) {
      let accuracy = calcAverage(accOverbought) * 100
      accMetric.push({
        type: 'Over Bought',
        symbol: ticker.symbol,
        accuracy: Math.round(accuracy),
        sampleSize: accOverbought.length
      })
    }

    if (accOversold.length) {
      let accuracy = calcAverage(accOversold) * 100
      accMetric.push({
        type: 'Over Sold',
        symbol: ticker.symbol,
        accuracy: Math.round(accuracy),
        sampleSize: accOversold.length
      })
    }
  })
